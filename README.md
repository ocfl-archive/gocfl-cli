...
GOCFL will be compiled with an embedded configuration file. It expects this
...
GOCFL is optimized next to the following Windows utilities and you will find
references to them under Indexer and Migration settings in the config toml file:

* `convert.exe` via [ImageMagick](https://imagemagick.org/)
* `identify.exe` via [ImageMagick](https://imagemagick.org/)
* `ffmpeg.exe` via [FFmpeg](https://ffmpeg.org/)
* `ffprobe.exe` via [FFmpeg](https://ffmpeg.org/)
* `gswin64.exe` via [Ghostscript](https://www.ghostscript.com/)
* `powershell.exe` via [Windows Powershell](https://learn.microsoft.com/en-us/powershell/)
...
This tool provides a command line interface for the Oxford Common Filesystem Layout ([OCFL][ocfl-1]) and
focuses on `creation`, `update`, `validation` and `extraction` of OCFL
StorageRoots and Objects using the [GOCFL library](https://github.com/ocfl-archive/gocfl).

[ocfl-1]: https://ocfl.io/

GOCFL supports the following subcommands
...
software is built with the following motivation (leveraging the [GOCFL library](https://github.com/ocfl-archive/gocfl)):
...
https://github.com/ocfl-archive/gocfl-cli
